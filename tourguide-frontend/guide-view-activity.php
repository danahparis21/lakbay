<?php
// guide/guide_view_activity.php - View past hike summary for guides
...
try {
    // Get guide's own guide_id
    $stmt = $pdo->prepare("SELECT id FROM guides WHERE user_id = ?");
    $stmt->execute([$currentUserId]);
    $guide = $stmt->fetch(PDO::FETCH_ASSOC);
    
    if (!$guide) {
        header('Location: guide-dashboard.php?error=not_guide');
        exit;
    }
    ...
    if (!empty($sessionId)) {
        $stmt = $pdo->prepare("
            SELECT ahs.*, b.mountain_id, m.name as mountain_name, m.trail_data,
                   b.booking_number, b.hike_date, b.hike_type, b.number_of_hikers
            FROM active_hike_sessions ahs
            JOIN bookings b ON ahs.booking_id = b.id
            JOIN mountains m ON b.mountain_id = m.id
            WHERE ahs.id = ? AND b.guide_id = ? AND ahs.status = 'finished'
        ");
        $stmt->execute([$sessionId, $guide_id]);
    } else {
        $stmt = $pdo->prepare("
            SELECT ahs.*, b.mountain_id, m.name as mountain_name, m.trail_data,
                   b.booking_number, b.hike_date, b.hike_type, b.number_of_hikers
            ...
            JOIN bookings b ON ahs.booking_id = b.id
            JOIN mountains m ON b.mountain_id = m.id
            WHERE ahs.booking_id = ? AND b.guide_id = ? AND ahs.status = 'finished'
            ORDER BY ahs.end_time DESC LIMIT 1
        ");
        $stmt->execute([$bookingId, $guide_id]);
    }

    $session = $stmt->fetch(PDO::FETCH_ASSOC);

    if (!$session) {
        header('Location: guide-dashboard.php?error=no_activity_found');
        exit;
    }
    ...
    $summary = [
        ...
        'hike_date'      => date('F j, Y', strtotime($session['hike_date'])),
        'hike_type'      => $session['hike_type'],
        'hikers'         => $session['number_of_hikers'],
...
<head>
    ...
    <title><?= htmlspecialchars($summary['mountain']) ?> — Guide Summary | LAKBAY</title>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .stat-col.big { flex: 1; }
        .stat-col.big .stat-val { font-size: 44px; letter-spacing: -2px; line-height: 1; }
        .stat-val {
        ...
        .stat-val-unit { font-size: 13px; font-family: 'DM Sans', sans-serif; color: var(--ink-4); font-weight: 500; margin-left: 2px; }
        .stat-lbl {
            font-size: 10px;
            font-weight: 600;
            text-transform: uppercase;
        ...
        .stat-right-stack { display: flex; flex-direction: column; justify-content: center; gap: 12px; }
        ...
        .info-label { font-size: 12px; font-weight: 600; color: var(--ink-4); text-transform: uppercase; letter-spacing: 0.5px; }
        .info-value { font-size: 13px; color: var(--ink); font-weight: 500; font-family: 'DM Mono', monospace; }
        ...
        .brand-name { font-size: 14px; font-weight: 800; color: var(--primary); letter-spacing: -0.3px; }
        .brand-sep { width: 1px; height: 14px; background: var(--line); }
        .brand-sub { font-size: 10px; color: var(--ink-4); font-weight: 500; }
        .btn-dashboard {
            display: inline-flex;
            align-items: center;
            gap: 6px;
...
<nav class="top-nav">
    <a href="guide-dashboard.php" class="back-btn">
...
<div class="activity-card" id="activityCard">
    ...
    <div class="card-hero">
        <div class="hero-eyebrow">
            <span class="hero-chip"><span class="hero-chip-dot"></span>Hike Completed</span>
            <span class="hero-date"><?= $summary['completed_at'] ?></span>
        </div>
        <div class="hero-mountain">⛰ <?= htmlspecialchars($summary['mountain']) ?></div>
        <div class="hero-sub">Finished at <?= $summary['completed_time'] ?> · Guide Summary</div>
    </div>
    ...
    <div class="stats-row">
        <div class="stat-col big">
            ...
            <div class="stat-lbl">Distance Led</div>
        </div>
    ...
    <div class="info-row">
        ...
        <span class="info-value"><?= htmlspecialchars($summary['booking_number']) ?></span>
    </div>
    <div class="info-row">
        <span class="info-label">Hike Date</span>
        <span class="info-value"><?= $summary['hike_date'] ?></span>
    </div>
    <div class="info-row">
        <span class="info-label">Hike Type</span>
        <span class="info-value"><?= ucfirst(str_replace('_', ' ', $summary['hike_type'])) ?></span>
    </div>
    <div class="info-row">
        <span class="info-label">Hikers Led</span>
        <span class="info-value"><?= $summary['hikers'] ?> person(s)</span>
    </div>
    <div class="info-row">
        <span class="info-label">Avg Speed</span>
        <span class="info-value"><?= $summary['avg_speed_kmh'] ?> km/h</span>
    </div>
    ...
    <div class="card-footer">
        <div class="brand-mark">
            ...
            <span class="brand-sub">Guided with Lakbay</span>
        </div>
        <a href="guide-dashboard.php" class="btn-dashboard">← Back to Dashboard</a>
    </div>
...
const hikersLed    = <?= $summary['hikers'] ?>;
const bookingNum   = <?= json_encode($summary['booking_number']) ?>;
...
async function shareActivity() {
    ...
    ctx.fillText('DISTANCE LED', PAD, sY + 68);
    ...
    ctx.fillStyle = 'rgba(16,6,0,0.45)';
    ctx.fillText('BOOKING #', PAD, startY + 12);
    ctx.font = '500 13px "DM Mono", monospace';
    ctx.fillStyle = '#100600';
    ctx.fillText(bookingNum, W - PAD - ctx.measureText(bookingNum).width, startY + 12);
    
    // Hike Date
    ctx.font = '500 12px "DM Mono", monospace';
    ctx.fillStyle = 'rgba(16,6,0,0.45)';
    ctx.fillText('HIKE DATE', PAD, startY + rowHeight + 12);
    ctx.font = '500 13px "DM Mono", monospace';
    ctx.fillStyle = '#100600';
    const hikeDate = "<?= $summary['hike_date'] ?>";
    ctx.fillText(hikeDate, W - PAD - ctx.measureText(hikeDate).width, startY + rowHeight + 12);
    
    // Hikers Led
    ctx.font = '500 12px "DM Mono", monospace';
    ctx.fillStyle = 'rgba(16,6,0,0.45)';
    ctx.fillText('HIKERS LED', PAD, startY + rowHeight * 2 + 12);
    ctx.font = '500 13px "DM Mono", monospace';
    ctx.fillStyle = '#100600';
    const hikersText = hikersLed + ' person(s)';
    ctx.fillText(hikersText, W - PAD - ctx.measureText(hikersText).width, startY + rowHeight * 2 + 12);
    
    // Avg Speed
    ctx.font = '500 12px "DM Mono", monospace';
    ctx.fillStyle = 'rgba(16,6,0,0.45)';
    ctx.fillText('AVG SPEED', PAD, startY + rowHeight * 3 + 12);
    ctx.font = '500 13px "DM Mono", monospace';
    ctx.fillStyle = '#100600';
    const speedText = "<?= $summary['avg_speed_kmh'] ?> km/h";
    ctx.fillText(speedText, W - PAD - ctx.measureText(speedText).width, startY + rowHeight * 3 + 12);
    ...
    ctx.fillStyle = 'rgba(16,6,0,0.35)';
    ctx.fillText('Guided with Lakbay', PAD + 70, fY + 24);
    ...
    canvas.toBlob(async (blob) => {
        ...
        if (mobile && navigator.share && navigator.canShare) {
            try {
                const file = new File([blob], 'lakbay_guide_hike.png', { type: 'image/png' });
                await navigator.share({ title: 'My Guided Hike', text: `I guided ${hikersLed} hiker(s) to ${mountainName}! 🏔️`, files: [file] });
...
async function copyToClipboard(blob) {
    try {
        ...
        const a = document.createElement('a'); a.download = 'lakbay_guide_hike.png'; a.href = url; a.click();
